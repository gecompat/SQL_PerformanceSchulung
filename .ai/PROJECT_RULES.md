...
- Die unter `.ai/foundation/` versionierte Foundation `1.8.0` bildet eine gemeinsame Mindestbasis. Ihre `REQUIRED`-Regeln dürfen nicht abgeschwächt werden; bewusst strengere Projektregeln bleiben zulässig und maßgeblich.
- Die historischen Bewertungen bis `1.7.0` stehen in [`FOUNDATION_UPGRADE_1_7_0_ASSESSMENT.json`](FOUNDATION_UPGRADE_1_7_0_ASSESSMENT.json); das vollständige Delta von `1.7.0` auf `1.8.0` steht in [`FOUNDATION_UPGRADE_1_8_0_ASSESSMENT.json`](FOUNDATION_UPGRADE_1_8_0_ASSESSMENT.json). `DEC-062` aktiviert weiterhin die zentrale Registry im Profil `foundation-artifact-registry/v2` und die Capability `artifact-registry-github` als projektführende Registrierungsautorität. Die optionale Capability `rule-context-cache` ist nicht ausgewählt; ohne späteren expliziten Projektentscheid bleibt der vollständige Lesepfad der sichere Standard.
