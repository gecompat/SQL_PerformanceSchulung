#!/usr/bin/env python3
"""Static contract validation for the ADV-008 OPT-009 slice."""
...
DEMO_ID = "OPT-009"
DEMO = ROOT / "Demos" / "04_Optimizer_Statistics_Plans" / "OPT-009_Parameter_Sensitive_Plans"
...
REQUIRED_MARKERS = {
    "lab.usp_Opt009SearchSelectiveFirst",
    "lab.usp_Opt009SearchCommonFirst",
    "lab.usp_Opt009SearchPsp",
    "lab.usp_Opt009SearchOptOut",
    "lab.usp_Opt009Capture",
    "lab.Opt009Evidence",
    "lab.PspOrder",
...
PHASE_PROCEDURES = {
    "20_Baseline.sql": "lab.usp_Opt009SearchSelectiveFirst",
    "30_Demonstration.sql": "lab.usp_Opt009SearchCommonFirst",
    "50_Mitigation.sql": "lab.usp_Opt009SearchPsp",
    "60_Comparison.sql": "lab.usp_Opt009SearchOptOut",
}
SLIDE_SPEC = ROOT / "Documentation" / "Curriculum" / "ADV_010_SLIDE_SPECIFICATION_M03_LO08_PSP.md"
SLIDE_IDS = ("SLD-M03-121", "SLD-M03-122", "SLD-M03-123", "SLD-M03-124")
SLIDE_CLAIMS = ("ADV-CLM-019",)
RUNNER = ROOT / "Tests" / "Runtime" / "run_adv008_opt009.py"
WORKFLOW = ROOT / ".github" / "workflows" / "adv008-opt009.yml"
...
def check_bundle(findings: list[str]) -> None:
    ...
    if "| Status | `VALIDATED` |" not in readme_text or "Actions-Lauf 30701731564" not in readme_text:
    ...
    if "QRY-004" not in readme_text or "OPT-010" not in readme_text:
        findings.append(f"{DEMO_ID}: README does not delimit the neighbouring demos")
    if "Gleichheitspr" not in readme_text:
        findings.append(f"{DEMO_ID}: README does not state the equality predicate limit")
...
def check_sql(findings: list[str]) -> None:
    ...
    if "@MajorVersion < 16" not in preflight or "SKIP_VERSION" not in preflight:
    ...
    if "COMPATIBILITY_LEVEL" not in setup:
        findings.append(f"{DEMO_ID}: setup does not pin the compatibility level")
    if "PARAMETER_SENSITIVE_PLAN_OPTIMIZATION = OFF" not in setup:
    ...
    if setup.count("CREATE PROCEDURE lab.usp_Opt009Search") != 4:
        findings.append(f"{DEMO_ID}: exactly four comparison procedures are expected")
    if setup.count("USE HINT ('DISABLE_PARAMETER_SENSITIVE_PLAN')") != 1:
    ...
    if "PARAMETER_SENSITIVE_PLAN_OPTIMIZATION = ON" not in mitigation:
    ...
    for name, text in (("20_Baseline.sql", read(DEMO / "20_Baseline.sql")),
                       ("30_Demonstration.sql", read(DEMO / "30_Demonstration.sql")),
...
def check_slide_specification(findings: list[str]) -> None:
    ...
    if "OPT-010" not in text:
        findings.append("slide specification: OPPO must stay out of scope explicitly")
...
def main() -> int:
    ...
    if findings:
        print(f"adv008-opt009: FAIL ({len(findings)} finding(s))")
        for finding in findings:
            print(f"- {finding}")
        return 1

    print(
        f"adv008-opt009: PASS ({len(EXPECTED_PHASES)} phases, {len(PHASE_PROCEDURES)} comparison objects, "
