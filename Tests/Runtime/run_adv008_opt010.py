#!/usr/bin/env python3
"""Run OPT-010 twice against one SQL Server execution target."""
...
DEMO_ID = "OPT-010"
MANIFEST = (
    ROOT / "Demos" / "04_Optimizer_Statistics_Plans"
    / "OPT-010_Optional_Parameter_Plans" / "manifest.json"
)
SUMMARY = re.compile(r"^SQLPERF_SUMMARY\|(PASS|WARN|SKIP|FAIL)\|([A-Z][A-Z0-9_]*)$", re.MULTILINE)
# Optional Parameter Plan Optimization setzt SQL Server 2025 voraus; auf 2019 und 2022
# ist SKIP_VERSION der fachlich vorgesehene Ausgang und darf die Matrix nicht rot faerben.
ACCEPTED_SKIPS = {
    "SKIP_VERSION",
    "SKIP_PERMISSION",
...
class Opt010Failure(RuntimeError):
    pass
...
def assert_database_absent(target: ExecutionTarget) -> None:
    ...
    output = execution_target.run_sql(
        ...
        sql_text=(
            ...
            "THROW 51004, 'FAIL_CLEANUP: OPT-010-Testdatenbank ist nach dem Harness-Lauf noch vorhanden.', 1; "
            "SELECT N'ABSENT';"
        ),
        timeout_seconds=30,
    )
    if "ABSENT" not in output:
        raise Opt010Failure(f"{DEMO_ID}: cleanup verification did not return ABSENT")


def run_demo(*, target: ExecutionTarget, repetition: int) -> tuple[str, str]:
    if not MANIFEST.is_file():
        raise Opt010Failure(f"{DEMO_ID}: manifest missing")
    ...
    print(f"OPT010_STAGE|{DEMO_ID}|RUN_{repetition}|{outcome}|{code}|warnings={variance}")
    return outcome, code


def main() -> int:
    parser = argparse.ArgumentParser(description="Run OPT-010 twice.")
    parser.add_argument(
        "--target",
    ...
    try:
        ...
        print(f"OPT010_STAGE|ENGINE_{major}|IDENTITY|PASS|OK")
        outcomes = [run_demo(target=target, repetition=repetition) for repetition in (1, 2)]
        if all(item[0] == "PASS" for item in outcomes):
            print(
                f"OPT010_SUMMARY|PASS|OK|major={major}; demos=1; repetitions=2; "
                f"target={target.kind}"
            )
        else:
            outcome, code = outcomes[-1]
            print(
                f"OPT010_SUMMARY|{outcome}|{code}|major={major}; demos=1; repetitions=2; "
                f"target={target.kind}"
            )
        return 0
    except (
        ...
        print(
            f"OPT010_SUMMARY|FAIL|FAIL_EXECUTION|major={major}; "
