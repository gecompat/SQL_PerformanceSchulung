/* OPT-016 / ADV-008 / FWK-001 / FWK-008 / FWK-012 */
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF @DemoId <> 'OPT-016'
   ...
   OR @TargetDatabase <> @ExpectedDatabase
    THROW 51000, 'FAIL_CONTRACT: Demo-ID, Run-Token oder Ziel-Datenbank entsprechen nicht dem OPT-016-Vertrag.', 1;
...
BEGIN
    SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'ENGINE_VERSION' AS CheckId,
           ...
           N'OPT-016 unterstützt SQL Server 2019 bis 2025.' AS Message;
...
BEGIN
    SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'PERMISSIONS' AS CheckId,
           ...
           N'Die Plan- und Runtime-Evidenz kann ohne diese Rechte nicht belastbar erhoben werden.' AS Message;
...
SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'SUMMARY' AS CheckId,
       ...
       CONCAT(N'Major=', @MajorVersion, N'; Safety=GREEN; LAST_QUERY_PLAN_STATS wird nur datenbankbezogen aktiviert') AS ObservedValue,
       N'SQL Server 2019 bis 2025; kanonische synthetische Datenbank' AS RequiredValue,
       N'Preflight für OPT-016 ist bestanden.' AS Message;
