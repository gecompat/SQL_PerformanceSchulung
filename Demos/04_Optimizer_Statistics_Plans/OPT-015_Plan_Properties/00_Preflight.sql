/* OPT-015 / ADV-008 / FWK-001 / FWK-008 / FWK-012 */
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF @DemoId <> 'OPT-015'
   ...
   OR @TargetDatabase <> @ExpectedDatabase
    THROW 51000, 'FAIL_CONTRACT: Demo-ID, Run-Token oder Ziel-Datenbank entsprechen nicht dem OPT-015-Vertrag.', 1;
...
BEGIN
    SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'ENGINE_VERSION' AS CheckId,
           ...
           N'OPT-015 unterstützt SQL Server 2019 bis 2025.' AS Message;
...
SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'SUMMARY' AS CheckId,
       ...
       N'Preflight für OPT-015 ist bestanden.' AS Message;
