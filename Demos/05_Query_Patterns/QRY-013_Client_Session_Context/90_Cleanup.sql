/* QRY-013 cleanup: drop only after complete FWK-002 marker verification. */
USE [master];
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF @DemoId <> 'QRY-013' OR @TargetDatabase <> @ExpectedDatabase
    THROW 51000, 'FAIL_CONTRACT: QRY-013-Cleanupziel ist ungültig.', 1;
...
SELECT 1 AS Sequence, 'CLEANUP' AS Phase, 'SUMMARY' AS CheckId, 'PASS' AS Outcome, 'OK' AS Code,
       N'markierte QRY-013-Datenbank entfernt' AS ObservedValue, N'vollständige Markerübereinstimmung' AS RequiredValue,
