/* QRY-004 / ADV-008 / FWK-001 / FWK-008 / FWK-012 */
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF @DemoId <> 'QRY-004'
   ...
   OR @TargetDatabase <> @ExpectedDatabase
    THROW 51000, 'FAIL_CONTRACT: Demo-ID, Run-Token oder Ziel-Datenbank entsprechen nicht dem QRY-004-Vertrag.', 1;
...
BEGIN
    SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'ENGINE_VERSION' AS CheckId,
           ...
           N'QRY-004 unterstützt SQL Server 2019 bis 2025.' AS Message;
...
BEGIN
    SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'PERMISSIONS' AS CheckId,
           ...
           N'Plan- und Cacheevidenz der drei Strategien ist ohne diese Rechte nicht belastbar auswertbar.' AS Message;
...
SELECT 1 AS Sequence, 'PREFLIGHT' AS Phase, 'SUMMARY' AS CheckId,
       ...
       CONCAT(N'Major=', @MajorVersion, N'; Safety=GREEN; Cacheauswertung ausschliesslich objekt- und markerbezogen') AS ObservedValue,
       N'SQL Server 2019 bis 2025; markergebundene synthetische Datenbank' AS RequiredValue,
       N'Preflight für QRY-004 ist bestanden.' AS Message;
