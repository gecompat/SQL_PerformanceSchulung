...
| Optimiertes `sp_executesql` | `SRC-058`, `SRC-059` | `QRY-004`, `ADV-CLM-018` | `DEFER` | Die Option ist standardmäßig aus und adressiert konkurrierende Compilation identischer Batches. `QRY-004` besitzt noch keine erneute Runtimefreigabe; erst danach darf ein Concurrent-Compile-Designschnitt bewertet werden. |
...
- Kein optimiertes `sp_executesql` vor belegter `QRY-004`-Runtimefreigabe.
