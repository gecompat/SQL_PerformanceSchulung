...
Die neun vorgesehenen Demos sind in der verbindlichen Reihenfolge als eigenständige, runtimefähige Pakete implementiert. Acht Demos sind nach je zwei lokalen Docker-Läufen auf SQL Server 2019, 2022 und 2025 `VALIDATED`. `CON-009` bleibt wegen einer fachlichen Evidenzlücke auf SQL Server 2019 `IMPLEMENTED`.
...
| 4 | `CON-009` | `IMPLEMENTED` | `SRC-029`, `SRC-004`, `SRC-031` `ACTIVE` | `YELLOW` | frische Performance-Instanz | `LO-M05-04`, `CLM-068`, `ADV-CLM-027` | User-/Internal-Objects und Version Store; Instanzkonfiguration zurückgestellt |
...
| `CON-009` | 2× `WARN_EMPIRICAL_VARIANCE` | 2× `PASS/OK` | 2× `PASS/OK` | 2019 erzeugte trotz begrenzter Sortierung keine positive interne Task-Allokation; keine Vollfreigabe |

## Noch offene Evidenz

- `CON-009` benötigt auf SQL Server 2019 positive interne Task-Allokation oder einen fachlich gleichwertigen, versionsrobusten Nachweis; die Warnung hält den Workflow grün, schließt aber die Runtimefreigabe nicht.
- Die lokale Matrix ist konkrete Runtime-Evidenz; der eingecheckte Workflow muss dieselben Verträge bei Pull Requests erneut prüfen.
