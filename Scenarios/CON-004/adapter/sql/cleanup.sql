/* Markergebundener, idempotenter Project-Adapter-Cleanup fuer CON-004. */
USE [master];
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
BEGIN
    SELECT N'CON-004' AS ScenarioId, N'CLEANUP' AS Phase, N'PASS' AS Outcome, N'ALREADY_ABSENT' AS Code;
    RETURN;
END;
...
IF @Project <> N'SQL_PerformanceSchulung' OR @Contract <> N'1.0' OR @Demo <> 'CON-004' OR @Run <> 'LOCAL'
    THROW 51004, 'PROJECT_CLEANUP_FAILED: CON-004-Eigentumsmarker stimmen nicht ueberein.', 1;
...
SELECT N'CON-004' AS ScenarioId, N'CLEANUP' AS Phase, N'PASS' AS Outcome, N'DATABASE_REMOVED' AS Code;
