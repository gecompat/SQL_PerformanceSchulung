/* Versionierter Project-Adapter-Install fuer den CON-004 READY_FOR_USER-Zustand. */
USE [master];
GO
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
BEGIN
    SET @Sql = N'USE ' + QUOTENAME(@TargetDatabase) + N';'
        ...
        + N'EXEC sys.sp_addextendedproperty @name=N''SQLPERF.DemoId'',@value=N''CON-004'';'
...
BEGIN
    ...
    IF @Project <> N'SQL_PerformanceSchulung' OR @Contract <> N'1.0' OR @Demo <> 'CON-004' OR @Run <> 'LOCAL'
        THROW 51002, 'ADAPTER_STATE_CONFLICT: Die vorhandene Datenbank besitzt nicht die CON-004-Eigentumsmarker.', 1;
END;
GO
...
CREATE TABLE lab.BlockingDemo
(
    BlockId int NOT NULL CONSTRAINT PK_BlockingDemo PRIMARY KEY,
    Value int NOT NULL
);
...
(
    ...
    HeadSessionId smallint NOT NULL,
    MiddleSessionId smallint NOT NULL,
    LeafSessionId smallint NOT NULL,
    MiddleBlockingSessionId smallint NOT NULL,
    LeafBlockingSessionId smallint NOT NULL,
    MiddleWaitType nvarchar(60) NOT NULL,
    LeafWaitType nvarchar(60) NOT NULL,
    MiddleWaitMs int NOT NULL,
    LeafWaitMs int NOT NULL,
    ChainDepth int NOT NULL,
    CapturedAtUtc datetime2(3) NOT NULL CONSTRAINT DF_BlockingEvidence_Captured DEFAULT SYSUTCDATETIME()
);
...
BEGIN
    ...
    WHILE NOT EXISTS
    (
        SELECT 1 FROM fwk.SessionSignal
         WHERE DemoId = @DemoId AND RunToken = @RunToken AND SignalName = @SignalName
    )
    BEGIN
        IF DATEDIFF_BIG(millisecond, @Start, SYSUTCDATETIME()) >= @TimeoutMs
            THROW 51005, 'FAIL_TIMEOUT: Erwartetes CON-004-Signal fehlt.', 1;
        WAITFOR DELAY '00:00:00.100';
    END;
END;
GO
CREATE OR ALTER PROCEDURE fwk.USP_ClearSignals
    @DemoId varchar(7), @RunToken varchar(20)
AS
BEGIN
    SET NOCOUNT ON;
    DELETE fwk.SessionSignal WHERE DemoId = @DemoId AND RunToken = @RunToken;
END;
GO
EXEC fwk.USP_ClearSignals @DemoId='CON-004', @RunToken='LOCAL';
...
IF EXISTS (SELECT 1 FROM sys.dm_exec_requests WHERE database_id = DB_ID() AND blocking_session_id <> 0)
    THROW 51002, 'FAIL_STATE: Vor READY_FOR_USER existiert Blocking in der Testdatenbank.', 1;
...
SELECT N'CON-004' AS ScenarioId, N'INSTALL' AS Phase, N'READY_FOR_USER' AS Outcome,
