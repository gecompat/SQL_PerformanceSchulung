/* Fachliche Adaptervalidierung fuer den reproduzierbaren CON-006-Ausgangszustand. */
USE [master];
GO
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF DB_ID(@TargetDatabase) IS NULL
    THROW 51002, 'PROJECT_ASSERTION_FAILED: CON-006-Datenbank fehlt.', 1;
...
IF @Project <> N'SQL_PerformanceSchulung' OR @Contract <> N'1.0' OR @Demo <> 'CON-006' OR @Run <> 'LOCAL'
    THROW 51002, 'PROJECT_ASSERTION_FAILED: CON-006-Eigentumsmarker stimmen nicht ueberein.', 1;
...
IF OBJECT_ID(N'lab.DeadlockRows', N'U') IS NULL
   OR OBJECT_ID(N'lab.Evidence', N'U') IS NULL
   ...
   OR OBJECT_ID(N'fwk.USP_WaitForSignal', N'P') IS NULL
    THROW 51002, 'PROJECT_ASSERTION_FAILED: CON-006-Objektvertrag ist unvollstaendig.', 1;
IF (SELECT COUNT(*) FROM lab.DeadlockRows) <> 2 OR EXISTS (SELECT 1 FROM lab.DeadlockRows WHERE Value <> 0)
    THROW 51002, 'PROJECT_ASSERTION_FAILED: CON-006-Baselinedaten sind ungueltig.', 1;
IF EXISTS (SELECT 1 FROM lab.Evidence) OR EXISTS (SELECT 1 FROM fwk.SessionSignal)
    THROW 51002, 'PROJECT_ASSERTION_FAILED: Alte CON-006-Evidenz oder Signale sind vorhanden.', 1;

SELECT N'CON-006' AS ScenarioId, N'VALIDATE' AS Phase, N'READY_FOR_USER' AS Outcome,
       DB_NAME() AS TargetDatabase, (SELECT COUNT(*) FROM lab.DeadlockRows) AS PreparedRows;
GO
